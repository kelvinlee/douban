...
$(document).ready(function() {
  readygo();
  return window.addEventListener('push', readygo);
});
...
myFunction = function(evt) {
  ...
  return $("#mySlider .slide-point span").eq(evt.detail.slideNumber).addClass('on');
};

closeAll = function() {
  return $(".pop,.model").removeClass('active');
};
...
signpop = function() {
  ...
  return $("#nologin").click(function(e) {
    if (!($(e.target).is(".popbody") || $(e.target).parents(".popbody").length > 0)) {
      return $("#nologin").removeClass('active');
    }
  });
};

gotopage = function(url, animate, anit) {
  if (animate == null) {
    animate = true;
  }
  if (anit == null) {
