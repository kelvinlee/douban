...
var ANIMATION_END_NAME, ANIMATION_END_NAMES, BindShare, LoadFinished, TRANSITION_END_NAME, TRANSITION_END_NAMES, VENDORS, app, beginload, css3Prefix, debug, fShare, handleFileLoad, i, loadList, mTestElement, my, preload, refreshShare, reloadWechat, shareFriend, shareTimeline, _Progress, _finishedlist, _i, _len, _tempH, _wechat, _wechat_bool, _wechat_f;
...
app.controller('HomeController', function($rootScope, $scope, $location, $timeout) {
  ...
  this.star = function() {
    return $location.path("/Divination");
  };
  return $rootScope.root = 'here';
});

app.controller('DivinationController', function($rootScope, $scope, $location) {
  var SHAKE_THRESHOLD, acc, last_update, next, shares, temp, tis;
  if ($rootScope.root !== "here") {
    $location.path("/");
  }
  temp = false;
  $(".divina3")[0].addEventListener(ANIMATION_END_NAME, function(evt) {
    if (!temp) {
      $scope.$apply(function() {
        return $location.path("/end/" + next);
      });
      return temp = true;
    }
  });
  ...
  return this.StarMotion(true);
});

app.controller('endController', function($rootScope, $scope, $location, $timeout, $route) {
  var goto;
  if ($rootScope.root !== "here") {
    $location.path("/");
  }
  ...
  return $scope.runPage = function(bool) {
    console.log(goto && !bool);
    if (goto && !bool) {
      goto = false;
      return $scope.$apply(function() {
        return $location.path("/over/" + $route.current.params.id);
      });
    }
  };
});

app.controller('overController', function($rootScope, $scope, $location, $timeout, $route) {
  var shares;
  if ($rootScope.root !== "here") {
    $location.path("/");
  }
  ...
  return this.show = function(bool) {
    return this.wechat = bool;
  };
});
