...
--------------------------------------------
 */
var BindShare, GameOver, Gametouchend, Gametouchmove, Gametouchstart, RSQRfade, RSQRfadeIn, RSQRfadeO, RSQRfadeOut, RSQfade, RSQfadeIn, ReadyStarGame, bindDecade, canvas, createRSQlist, detouchend, detouchmove, detouchstar, fShare, fadeOutSDBG, fadeOutSDBGFun, getParam, getdata64, handleComplete, handleFileLoad, hideload, hideloadAnimate, init, isScrolling_move, loaded, localimg, manilsit, moveDeBG, moveNextDecade, movePrevDecade, myFunction, oldMidPt, oldPt, openGameNextStep, openMessage, percent2radians, postajax, postajaxPC, preload, removeListenter, rsqarr, selectRSQTo, showFirstFrame, showSecondFrame, stage, starBG, starBGshow, startGame, tick, updateCacheImage, _cmh, _decade, _dom, _fromPC, _lsxm, _oldy, _openMessage, _remendIndex, _rsqlfx, _shareimgurl;
...
percent2radians = function(percent) {
  return percent * Math.PI * 2;
};

init = function() {
  var cvs, h, l, photoid, userid, w;
  window.scrollTo(0, 1);
...
getdata64 = function() {
  ...
  console.log(dataURL);
  if (_fromPC) {
    return postajaxPC(dataURL);
  } else {
    return postajax(dataURL);
  }
};
...
postajax = function(data) {
  return $.ajax({
    ...
    success: function(msg) {
      ...
      return BindShare("有些话不愿大声说出来，不妨就写在充满雾气的浴室玻璃上吧！那些话，隐秘而伟大。", "http://site.douban.com/haier_waterheater/", _shareimgurl);
    },
    error: function(msg) {
      $("#uploadsuccess").attr('src', 'img/icon-success.png');
      return $("#uploadsuccesstext").html("上传失败了<br/>试试再玩一次！");
    }
  });
};

postajaxPC = function(data) {
  var photoid, userid;
  photoid = getParam("photo_id");
  userid = getParam("user_id");
  return $.ajax({
...
bindDecade = function() {
  ...
  $(".decade .prev").click(function() {
    return movePrevDecade();
  });
  $(".decade .next").click(function() {
    return moveNextDecade();
  });
  return $("#homenext").click(function() {
    return showSecondFrame();
  });
};
...
detouchstar = function(evt) {
  evt.preventDefault();
  return _oldy = evt.touches[0].pageY;
};

detouchmove = function(evt) {
  evt.preventDefault();
  if ((evt.touches[0].pageY - _oldy) > _cmh) {
    moveNextDecade();
  }
  if ((evt.touches[0].pageY - _oldy) < -_cmh) {
    return movePrevDecade();
...
detouchend = function(evt) {
  $(".decade .box")[0].addEventListener('touchstart', detouchstar);
  return $(".decade .box")[0].addEventListener('touchmove', detouchmove);
};

removeListenter = function() {
  $(".decade .box")[0].removeEventListener('touchstar', detouchstar);
  return $(".decade .box")[0].removeEventListener('touchmove', detouchmove);
};

moveNextDecade = function() {
  var $e, index;
  removeListenter();
  $e = $(".decade .boxlist .on").prev();
  if ($e.is(".item") && $e.index() !== 0) {
    $(".decade .boxlist .item").removeClass('on');
    $e.addClass('on');
    index = $e.index() - 1;
    $(".decade .boxlist").css({
      "-webkit-transform": "translate3d(0px, -" + index * 2 + "rem, 0px)"
    });
    return moveDeBG(index);
  }
};

movePrevDecade = function() {
  var $e, index;
  removeListenter();
...
starBGshow = function() {
  ...
  if (_dom.bgdom.alpha >= 1) {
    ...
    starBG = false;
    $(".homepage").show();
    return $(".selectde").removeClass('hidethis');
  }
};

showSecondFrame = function() {
  ...
  return setTimeout(function() {
    return $(".selectres").addClass('showthis');
  }, 1);
};

selectRSQTo = function(e) {
  var index;
  $(".selectres").removeClass('showthis');
  setTimeout(function() {
    return $(".selectres").hide();
  }, 200);
  ...
  RSQfade = true;
  RSQRfade = true;
  return _dom.rsqrdom.addEventListener('click', ReadyStarGame);
};

RSQfadeIn = function() {
  _dom.rsqdom.alpha += 0.05;
  if (_dom.rsqdom.alpha >= 1) {
    return RSQfade = false;
  }
};
...
RSQRfadeIn = function() {
  ...
  if (_dom.rsqrdom.alpha >= 1) {
    return RSQRfade = false;
  }
};

RSQRfadeOut = function() {
  RSQRfade = false;
  _dom.rsqrdom.y += _dom.rsqrdom.My;
  _dom.rsqrdom.alpha -= 0.05;
  if (_dom.rsqrdom.alpha <= -1) {
    return RSQRfadeO = false;
  }
};

fadeOutSDBGFun = function() {
  _dom.sdbg.alpha += 0.01;
  if (_dom.sdbg.alpha >= 1) {
    fadeOutSDBG = false;
    return openGameNextStep();
  }
};

createRSQlist = function() {
  var $tm, i, item, items, _i;
  if ($(".rxqlist .slide").length >= rsqarr.length) {
    return false;
  }
  for (i = _i = 1; _i <= 15; i = ++_i) {
    items = $("<div>").addClass('slide');
    if (i % 3 === 0 || i === 1) {
      $(".rxqlist .slide-group").append(items);
    }
  ...
  return $(".rxqlist .next").click(function() {
    var lw;
    if (_lsxm === $("#mySlider .slide").length - 1) {
      return false;
    }
    lw = $("#mySlider").width();
    $("#mySlider .slide-group").css({
...
myFunction = function(evt) {
  return _lsxm = evt.detail.slideNumber;
};
...
openMessage = function() {
  ...
  return _openMessage = setTimeout(function() {
    return $(".gamestar").addClass("hidethis");
  }, 4000);
};

openGameNextStep = function() {
  return $(".gamestarpage").show();
};
...
startGame = function() {
  ...
  $("#reset").click(function() {
    ...
    return updateCacheImage(true);
  });
  return $("#gameend").click(function() {
    return GameOver();
  });
};
...
GameOver = function() {
  ...
  $("#sharebtns").click(function() {
    ...
    return $(".gameend").addClass('showshare');
  });
  return $("[data-weixin]").click(function() {
    return $(".weixinshare").show();
  });
};
...
BindShare = function(content, url, pic) {
  ...
  return $("[data-share]").unbind('click').bind('click', function() {
    return fShare(list[$(this).data('share')], content, url, pic);
  });
};
...
Gametouchstart = function(evt) {
  console.log(evt);
  oldPt = new createjs.Point(stage.mouseX, stage.mouseY);
  return oldMidPt = oldPt;
};

Gametouchend = function(evt) {
  return updateCacheImage(true);
};

Gametouchmove = function(evt) {
  var midPoint;
  console.log(oldPt, oldMidPt);
  midPoint = new createjs.Point(oldPt.x + stage.mouseX >> 1, oldPt.y + stage.mouseY >> 1);
  _dom.sdbgmark.graphics.setStrokeStyle(40, "round", "round").beginStroke("rgba(0,0,0,0.85)").moveTo(midPoint.x, midPoint.y).curveTo(oldPt.x, oldPt.y, oldMidPt.x, oldMidPt.y);
...
ReadyStarGame = function() {
  RSQRfadeO = true;
  return startGame();
};

hideloadAnimate = function() {
  if (_dom.load.alpha < -0.1) {
    hideload = false;
    showFirstFrame();
  }
  return _dom.load.alpha -= 0.03;
};
...
tick = function(e) {
  ...
  if (_dom.bgdom != null) {
    _results = [];
    for (i = _i = 0, _ref = _dom.bgdom.getNumChildren(); 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
      ...
      if (Math.abs(temp.y - temp.My) <= 50 * 1.5) {
        _results.push(temp.y = temp.My);
      } else {
